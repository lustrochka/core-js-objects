...
function makeWord(lettersObject) {
  const res = [];
  const entries = Object.entries(lettersObject).flat(2);
  let letter;
...
function sellTickets(queue) {
  ...
  return !queue.some((cent) => {
    switch (cent) {
      case 50:
        if (money[25] === 0) return true;
        money[cent] += 1;
        money[25] -= 1;
        break;
      case 100:
        if (money[25] === 0 || money[50] === 0) return true;
        money[25] -= 1;
        money[50] -= 1;
        break;
      default:
...
function group(array, keySelector, valueSelector) {
  return array.reduce((acc, value) => {
    if (acc.get(keySelector(value))) {
      const arr = acc.get(keySelector(value));
      arr.push(valueSelector(value));
      acc.set(keySelector(value), arr);
    } else {
